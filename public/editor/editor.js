...
lnTools = ace.require("ace/ext/language_tools"), keywords = {
    grid: 108210,
    series: 1161721,
    pie: 173151,
    legend: 278319,
...
function makeSearch(e) {
    var t = [];
    for (var o in e) null != e[o] && t.push(o + "=" + e[o]);
    return t.join("&")
}
...
_.each((location.search || "").substr(1).split("&"),
function(e) {
    var t = e.split("=");
    configs[t[0]] = t[1]
});
...
function initEditor(domId,mode = 'javascript',data) {

    var editorDom = document.querySelector(`#${domId}`);
    if(!editorDom) throw '请传入正确的DOM id';

    var editorStyle = getComputedStyle(editorDom,null);
    if(editorStyle.position != 'absolute'){
        editorDom.style.position = 'relative';
    }

    if(editorStyle.width == '0px'){
        editorDom.style.width = '600px';
    }
    if(editorDom.style.height == '0px' || editorDom.style.height == '0'){
    ...
    gb.editor = ace.edit(domId);
    gb.editor.getSession().setMode("ace/mode/"+mode);
    ...
    document.querySelector('.ace-tm .ace_print-margin').style.width = '0';
}
...
function _clearTimeTickers() {
    ...
    for (e = 0; e < _timeoutIdList.length; e++) clearTimeout(_timeoutIdList[e]);
    _intervalIdList = [],
    _timeoutIdList = []
}
window.setTimeout = function(e, t) {
    var o = _oldSetTimeout(e, t);
    return _timeoutIdList.push(o),
    o
},
window.setInterval = function(e, t) {
    var o = _oldSetInterval(e, t);
    return _intervalIdList.push(o),
    o
};
var _events = [];
function _wrapOnMethods(o) {
    var n = o.on;
    o.on = function(e) {
        var t = n.apply(o, arguments);
        return _events.push(e),
        t
    }
}
function _clearChartEvents() {
    _events.forEach(function(e) {
        gb.chart && gb.chart.off(e)
    }),
    _events.length = 0
}
...
function localReset() {
    gb.loadedCode && (gb.editor.setValue(gb.loadedCode), gb.enableLive || runDebounce(), localSave(), $("#reset-btn").hide(), run())
}
function hasEditorError() {
    for (var e = gb.editor.getSession().getAnnotations(), t = 0, o = e.length; t < o; ++t) if ("error" === e[t].type) return ! 0;
    return ! 1
}
function load() {
    var e = configs.gl ? "data-gl": "data";
    if (configs.gl) {
        var t = document.createElement("script");
        t.onload = function() {
            o()
        },
        t.src = "../vendors/echarts-gl/echarts-gl.js",
        document.body.appendChild(t)
    } else o();
    function o() {
        configs.c && (console.log(configs.c), $.ajax("../" + e + "/" + configs.c + ".js", {
            dataType: "text",
            success: function(e) {
                gb.loadedCode = e,
                gb.editor.setValue(e, -1),
                gb.enableLive || runDebounce()
            }
        }).fail(function() {
            log("加载图表失败！", "error")
        }))
...
function log(e, t) {
    var o = formatTime(new Date);
    "warn" !== t && "error" !== t && (t = "info"),
    $("#code-info").html('<span class="code-info-time">' + o + '</span><span class="code-info-type-' + t + '">' + e + "</span>")
}
function formatTime(e) {
    for (var t = [e.getHours(), e.getMinutes(), e.getSeconds()], o = "", n = 0, a = t.length; n < a; ++n) o += (t[n] < 10 ? "0": "") + t[n],
    n < a - 1 && (o += ":");
    return o
}
function getVersion() {
